...
- [x] Designed a minimal CLI subcommand set: `aimem-inspect list` (list entities/live observations in the current project's `.aimem/memory.db`), `aimem-inspect search <query>` (same hybrid search `memory_search` uses, callable without an MCP client), `aimem-inspect export` (dump the full graph — including invalidated observations — to JSON for backup/migration), plus `aimem-inspect repair [--yes]` (the corruption-recovery confirmation flow deferred from Phase 9A/9E, see ADR-018/ADR-022).
- [x] Implemented as a genuinely separate entry point (`src/cli/inspect.ts`) that reuses `StorageEngine`/`RetrievalEngine`/`EmbeddingEngine`/`recovery.ts` directly — no MCP protocol involved, no new tool exposed to AI clients, purely a human-facing local utility. Pure logic (`runList`/`runSearch`/`runExport`/`runRepair`) is separated from the `main()`/process.exit CLI wiring so it's unit-testable without spawning a subprocess.
- [x] Wired a second `bin` entry, `aimem-inspect` (`dist/cli/inspect.js`), rather than a subcommand dispatch inside the existing `aimem` binary — the `aimem` binary is a pure stdio JSON-RPC MCP server with zero argv parsing by design; adding CLI arg handling to it would mix two very different concerns. Reused the exact `isMainModule`/`realpathSync` symlink-safety pattern from `server.ts` (ADR-010) since this binary faces the identical global-install/npx symlink invocation path.
- [x] Added `getAllObservationsByEntity` to `StorageEngine` (Phase 9F's `getObservationsByEntity` now excludes invalidated rows, but a backup/migration export should preserve full history) — a small additive method, not a change to existing behavior.
- [x] Wrote tests: real temp `.aimem/memory.db` via `StorageEngine` directly, one test per subcommand's pure-logic function (list/search/export/repair), including the invalidated-observations-excluded-from-list-but-included-in-export distinction and the repair-requires-explicit-confirmation flow. Manually smoke-tested the compiled CLI end-to-end (all four subcommands, including a real backup-and-restore round trip) before writing the automated tests.
- [x] Updated [knowledge/setup/usage-guide.md](../knowledge/setup/usage-guide.md) with real usage examples for all four subcommands.
