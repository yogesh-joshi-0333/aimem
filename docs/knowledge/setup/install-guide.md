...
2. Install aimem globally: `npm install -g aimem`
   - Alternatively, run without installing: `npx aimem` (downloads and runs on demand, per npx's normal caching behavior; the embedding model itself is still bundled in the package, not downloaded separately at runtime).
3. Register aimem as an MCP server in your AI client's MCP configuration, pointing the command at the installed binary (e.g. `aimem` if globally installed, or `npx aimem` otherwise). Consult your specific client's MCP server configuration documentation for the exact config file location.
