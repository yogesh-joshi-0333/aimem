...
**Backup and recovery (Phase 9A):** `StorageEngine` copies `.aimem/memory.db` to a single rolling backup (`.aimem/memory.db.bak`, no history — see `src/storage-engine/backup.ts`) before two risky operations: (1) running migrations on an already-existing file (inside the constructor, before `runMigrations()`), and (2) `ConflictVersioningEngine.confirmUpdate`'s confirmed-update write. This is deliberately *not* on the `memory_store`/`memory_scan` hot path — only before writes that could plausibly corrupt or lose data if interrupted. There is no in-place SQLite repair path (`better-sqlite3` has no equivalent to the `sqlite3` CLI's `.recover` dot-command, and shelling out to a system `sqlite3` binary would violate the zero-external-dependency principle — see `src/storage-engine/recovery.ts`). The only recovery mechanism is restoring from this backup, which requires the user's explicit confirmation via the `aimem inspect repair` CLI command (Phase 9D) — never automatic, per Rule 11.
