...
Each search independently returns up to `max(limit, MIN_HYBRID_CANDIDATE_POOL_SIZE)` candidates (the pool floor is 20, but it scales up for larger requested limits — fixed during a post-launch hardening pass; a fixed pool smaller than `MAX_SEARCH_LIMIT` used to structurally cap results below what was requested even with enough distinct live candidates available); RRF combines them by **rank position**, not raw score — `sqlite-vec`'s distance and FTS5's bm25-based `rank` are not on a comparable scale (verified empirically: FTS5's `rank` is an unbounded small-negative number whose magnitude depends on corpus statistics), so position-based fusion sidesteps that mismatch rather than trying to weight two incompatible units together. A result ranking well in *either* list scores highly; a result ranking well in *both* scores higher than either alone. Both underlying searches also exclude invalidated observations (Phase 9F) directly in SQL, not as a post-filter — necessary because `sqlite-vec`'s k-nearest-neighbor cutoff happens before any JOIN/WHERE filter would apply, so post-filtering could zero out live results that existed just outside the raw KNN window. See `src/embedding-search-engine/hybrid-search.ts`, `vector-index.ts`, `keyword-search.ts`.
