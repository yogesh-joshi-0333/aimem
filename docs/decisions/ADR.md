...
## ADR-020: Explicit Stale-Fact Invalidation via a Dedicated `memory_invalidate` Tool

**Date:** 2026-08-07
**Status:** Accepted

**Decision:** Added the ability to mark an observation as no longer true, with no replacement value, via a new `observations.invalidated_at` nullable column (migration `005-observation-invalidation.sql` plus a JS-gated `ALTER TABLE`, see below) and `ConflictVersioningEngine.invalidate(observationId)`. Exposed as a new, dedicated MCP tool, `memory_invalidate(observation_id)`, rather than folding into `memory_confirm_update` as a third `action` value as Phase 9F's original task wording suggested. `memory_search` and `memory_get_project_context` now exclude invalidated observations; `getVersionHistory` remains unfiltered so an invalidated fact stays queryable.

**Reason:** `memory_confirm_update` is keyed on `conflict_id` — it resolves a specific pending row in the `conflicts` table, which only exists because a *new* conflicting value was proposed. Invalidation has no new value and no conflict: the caller only has an `observation_id`. Mixing both entry points into one schema (`conflict_id` sometimes, `observation_id` other times, selected by an `action` enum) would have made the tool's input contract ambiguous and harder to validate correctly with JSON Schema. A small dedicated tool keeps each schema's required fields unambiguous and matches the project's existing precedent of one tool per distinct action (`memory_store` vs. `memory_scan` vs. `memory_remember` are already split this way despite all three ultimately writing observations). This raises the total tool count from 6 to 7 — a deliberate, documented deviation from the original Phase 9F task wording's "keep the tool count at 6" goal, made because correctness of the input contract took priority over holding a specific tool count.

A second empirical gotcha surfaced during implementation, the same class of problem as the FTS5 backfill gotchas in ADR-019: SQLite's `ALTER TABLE ADD COLUMN` has no `IF NOT EXISTS` form, but `StorageEngine`'s migration files all re-execute on every server startup (they rely on `CREATE TABLE/INDEX IF NOT EXISTS` to be safe to re-run). An unconditional `ALTER TABLE observations ADD COLUMN invalidated_at TEXT` inside a `.sql` migration file would throw `"duplicate column name"` on the second and every subsequent startup. Moved into JS as `addInvalidatedAtColumnIfNeeded()`, gated by checking `PRAGMA table_info(observations)` first — mirroring the same JS-side conditional-gating pattern `backfillFtsIndexIfNeeded()` established in Phase 9E for exactly the same underlying reason (some SQLite DDL operations have no idempotent form and must be gated in application code instead of pure SQL).

**Consequences:** `memory_confirm_update`'s existing schema and behavior are completely unchanged — this is a purely additive change with no migration risk to the conflict-resolution path. The new column-add step (`addInvalidatedAtColumnIfNeeded`) must run after the core migrations (so `observations` already exists on a fresh database) and before `005-observation-invalidation.sql` (whose index is defined over the new column) — `StorageEngine`'s migration sequence is now `runCoreMigrations()` → `addInvalidatedAtColumnIfNeeded()` → `runPostInvalidationMigrations()` → `backfillFtsIndexIfNeeded()`, an ordering constraint future migrations that add columns to existing tables will need to respect. `invalidate()` reuses `archiveVersion` with `oldVersion === newVersion` (no version bump) to record the pre-invalidation value in history — a version history row whose `superseded_by_version` equals its own `version` is the signal that the fact was invalidated, not superseded by a newer value, distinguishable from a normal `memory_confirm_update` archive entry. 97 fast tests + 10 e2e tests passing (up from 85/9 before 9F).
...
## ADR-023: Pre-Publish Hardening Pass — Input Size Ceilings, Hybrid Search Pool Fix, Offline Postinstall Guarantee

**Date:** 2026-08-07
**Status:** Accepted

**Decision:** Immediately before the first public npm/GitHub publish, ran a dedicated security/bug audit and fixed everything real it found:

1. Added `maxLength` (`MAX_NAME_LENGTH`=256, `MAX_OBSERVATION_LENGTH`=8000) to every free-text field and `maxItems` (`MAX_SCAN_CANDIDATES`=100) to `memory_scan`'s candidates array, across all 7 MCP tool schemas in `tool-schemas.ts`. Previously every string/array field was unbounded.
2. Fixed a real bug in hybrid search's invalidated-observation handling: `searchSimilar` (`vector-index.ts`) and `searchKeyword` (`keyword-search.ts`) now exclude invalidated observations (Phase 9F) directly in SQL, instead of `retrieval-engine.ts` filtering them out only after the per-mode candidate pool was already truncated — which could zero out live results entirely when enough candidates were invalidated (reproduced empirically: 10 observations, 8 invalidated, requesting k=5 returned 0 results, not the 2 live ones that existed). `sqlite-vec`'s `k = ?` KNN constraint applies *before* any JOIN/WHERE filter, so `vector-index.ts` over-fetches a wider KNN window (`KNN_OVERFETCH_MULTIPLIER = 4`) and filters+truncates in JS; FTS5's plain `ORDER BY`+`LIMIT` composes safely with a `WHERE` filter, so `keyword-search.ts` filters directly in SQL.
3. Fixed `HYBRID_CANDIDATE_POOL_SIZE` (fixed at 20) being smaller than `MAX_SEARCH_LIMIT` (50) — a `limit=50` request could structurally never return more than 20 results even with 50+ distinct live candidates available. Renamed to `MIN_HYBRID_CANDIDATE_POOL_SIZE` and changed `searchHybrid` to use `max(limit, floor)` as the actual per-mode pool size.
4. Made `scripts/bundle-embedding-model.mjs` (the `postinstall` hook, which runs on every end user's `npm install`, not just the maintainer's own bundling step) refuse to fall back to downloading the model from Hugging Face Hub if the bundled `models/.bundled` marker is missing. It now requires an explicit `AIMEM_BUNDLE_MODEL=1` env var (set only by the maintainer, manually, before `npm pack`/`npm publish`) to ever import `@xenova/transformers` and hit the network; otherwise it exits 1 with a clear message pointing at a corrupted/incomplete install.
5. `StorageCorruptedError`'s message no longer interpolates the absolute `dbPath` (moved to a structured field instead) — defense-in-depth, since `tool-router.ts` currently only branches on `instanceof` and never reads `.message` for this error, but a future refactor that logs or forwards `.message` should not be able to leak an absolute filesystem path across the MCP tool-call boundary (FR-ERR-04).

**Reason:** The project owner's explicit instruction before today's publish: "bug, issue, and any security must be not in the new version." Rather than assume the existing test suite already covered this, a fresh audit was run against exactly the areas a first public release is most likely to be attacked or embarrassed by (unbounded input → DoS, a documented "fully offline" claim with a live network-fallback gap, a genuine ranking bug from very recent Phase 9F/9E work interacting badly together). Every finding here was verified empirically (the invalidated-observation bug was reproduced with a real script showing 0 results returned; the offline fallback was traced through the exact `postinstall` lifecycle hook and confirmed it would import a network-capable library on a broken/incomplete install) rather than reported speculatively.

**Consequences:** `retrieval-engine.test.ts`'s `MAX_SEARCH_LIMIT` test was tightened from `toBeLessThanOrEqual(50)` back to an exact `toBe(50)`, since the underlying pool-size bug this looser assertion was accommodating is now fixed. A new regression test proves invalidated observations no longer starve the candidate pool (10 observations, 8 invalidated, requesting limit=5 now correctly returns exactly the 2 live ones). `vector-index.ts`'s over-fetch multiplier means a `memory_search` call does slightly more work per query (4x the requested limit fetched from `vec_observations` before filtering) — an accepted, cheap tradeoff at this project's target scale (thousands of observations, not millions). The `AIMEM_BUNDLE_MODEL=1` env var is a maintainer-only manual step, documented here and in the script's own comment, that must be run before every `npm pack`/`npm publish` if the model needs re-bundling (e.g. after ADR-017's `bge-small-en-v1.5` evaluation, which required exactly this). 10 new tests added directly against the real JSON schemas (`tool-schemas.test.ts`) proving every size ceiling is actually enforced by ajv, not just declared. 134 fast tests + 10 e2e tests passing. Also verified via a real `npm pack` + tarball install in a scratch directory that `postinstall` completes without any network access and both `aimem`/`aimem-inspect` binaries work correctly post-install.
